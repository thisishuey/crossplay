...
# FORK CHANGE (thisishuey): the invariant is inverted here. Upstream moved
# landing and publishing to scripts_local/ship.sh on one Mac and made this
# workflow a nightly audit (#232). This fork has no such machine: its device
# build IS this workflow on push and pull request, and its releases are cut by
# crossplay-release.yml on a v* tag. So a missing blocking trigger is the
# regression here, and the blocks guarded by BLOCKING_TRIGGER below stay live.
checks=$((checks + 1))
if [ "$BLOCKING_TRIGGER" != yes ]; then
  failed=$((failed + 1))
  echo "FAIL ci  crossplay-ci.yml has no push or pull_request trigger. In this fork it is the only device build a change gets before it lands; a nightly-only workflow would let xteink go unbuilt until the next morning."
fi
